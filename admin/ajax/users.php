...
if (isset($_POST['get_users'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $verified = "<span class='badge rounded-pill bg-warning'><i class='bi bi-x-lg'></i></span>";
        if ($row['is_verified']) {
            $verified = "<span class='badge rounded-pill bg-success'><i class='bi bi-check-lg'></i></span>";
            $del_btn = '';
        }
        ...
        $data .= "
          <tr>
            <td>$i</td>
            <td>
            <img src='$path$row[profile]' width='50px' /><br>
           $row[name]</td>
            <td>$row[email]</td>
            <td>$row[phonenum]</td>
...
if (isset($_POST['remove_user'])) {
    $frm_data = filteration($_POST);
    $pre_q = "SELECT * FROM `user_cred` WHERE `id`=? AND `is_verified`=?";
    $res1 = select($pre_q, [$frm_data['user_id'],0], 'ii');
    $img = mysqli_fetch_assoc($res1);
    if(deleteImage($img['profile'], USERS_FOLDER)){
